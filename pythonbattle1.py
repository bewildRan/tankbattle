...
other robot's turn."""
    ...
    def upd(self):
        ...
        p1 = self.position
        for i in addHPs:
#            if i.position[0] == p1[0] and i.position[1] == p1[1]: 
            if i.position == p1:
                self.health += i.dHP
                addHPs.remove(i)
        for i in addATKs:
...
def drawBattlefieldPygame(bot1, bot2):
    ...
    global state, redsquares, bluesquares, bot1img, bot2img, addHPimg, addSPDimg, addATKimg, namefont, ai1name, ai2name, BLKs, addHPs, addATKs, addSPDs
    ...
    for i in BLKs:
        pygame.draw.rect(screen,(64,64,64),(((i.position[0]-1)*48,(i.position[1]-1)*48),(48,48)))
    for i in addHPs:
    ...
    pygame.draw.rect(screen,(60,40,3),((  5*48,5*48  ),(48,48)))
    #draw grid lines
    for i in range(1,10):
    ...
    if (5,5) in redsquares:
        lred += 15
    elif (5,5) in bluesquares:
        lblue += 15
    ...
    pygame.draw.rect(screen,(0,255,0),((480,60),(int(bot1.health*160/100.0),10)))
    ...
    screen.blit(namefont.render("#blueSqs"+ str(lblue),True,(255,0,0),(0,0,0)),(480,140))
    #Draw the robot's health bars
    
    pygame.draw.rect(screen,(0,255,0),((480,160),(int(bot2.health*160/100.0),10)))
    pygame.display.flip()
    #If the game is over...
    if state == "win":
        running = True
...
redsquares = []
bluesquares = []
addHPs = [addHP((random.randint(0,10),random.randint(0,10)),1)]
addATKs = [addATK((random.randint(0,10),random.randint(0,10)),1)]
addSPDs = [addSPD((random.randint(0,10),random.randint(0,10)))]
state = "red"
...
addATKimg = pygame.image.load("addATK.png").convert_alpha()
...
while state != "win":
    ...
    if temp <= 0.05:
        pc = (random.randint(0,10), random.randint(0,10))
        while chk(pc):
            pc = (random.randint(0,10), random.randint(0,10))

...
#            self.health -= 1000
        ...
        p1=self._spaceInFront()
        objFront = self._getSpace(p1)
        if objFront == "wall" or objFront == "bot":
            return objFront
...
#            self.health -= 1000
        ...
        if self.rotation == 0:
            p1 = (self.position[0],self.position[1]+1)
        elif self.rotation == 1:
            p1 = (self.position[0]-1,self.position[1])
        elif self.rotation == 2:
            p1 = (self.position[0],self.position[1]-1)
        elif self.rotation == 3:
            p1 = (self.position[0]+1,self.position[1])
...
while state != "win":
    ...
    for i in field:
        state = i.name    #give turn default    add 2018年1月4日 by mhj
    ...
    #judge win after a full turn 
    for i in field:
        if i.health<=0:
            state="win"
            
    numberOfTurns += 1
    if numberOfTurns >= 500 and state!="win":
...
if state == "stalemate":
    ...
    else:   #all died,judged by the squares
        (lred, lblue) = calSqs(redsquares, bluesquares)
        if  lred> lblue:
            print("The winner is Red with",lred,"squares!")
            print("(Blue had",lblue,"squares)")
